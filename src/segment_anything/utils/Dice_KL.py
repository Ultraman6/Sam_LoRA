...
class DiceCELossWithKL(_Loss):
    ...
    # 原始pred, self的pred, target
    def forward(self, input: torch.Tensor, _input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """
        ...
        dice_loss = self.dice(_input, target)
        ce_loss = self.ce(_input, target) if _input.shape[1] != 1 else self.bce(_input, target)
        preds = F.softmax(input, dim=1)
        _preds = F.softmax(_input, dim=1)
        ...
        _pred_proportion = self.calculate_pred_proportion(_preds)
        kl_loss = self.kl_div(pred_proportion, _pred_proportion)
